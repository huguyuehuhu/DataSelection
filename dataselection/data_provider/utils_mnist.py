...
class DataSet(object):
    def __init__(self,
        ...
        # Convert/data/gyhu/code/dataselection/dataselection1 shape from [num examples, rows, columns, depth]
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            labedef load_Optdigits_dataset(path='./dataset/Optdigits/'):
    optdigits_train = pd.read_csv(path+'optdigits.tra.txt', header=None)
    input = shuffle(optdigits_train)
    x_train = input[np.arange(input.shape[1] - 1)]
    y_train = input[input.shape[1] - 1]

    optdigits_tedef load_Optdigits_dataset(path='./dataset/Optdigits/'):
    optdigits_train = pd.read_csv(path+'optdigits.tra.txt', header=None)
    input = shuffle(optdigits_train)
    x_train = input[np.arange(input.shape[1] - 1)]
    y_train = input[input.shape[1] - 1]

    optdigits_test = pd.read_csv(path + 'optdigits.tes.txt', header=None)
    input = shuffle(optdigits_test)
    x_test = input[np.arange(input.shape[1] - 1)]
    y_test = input[input.shape[1] - 1]
    return DataSet(x_train, x_test, y_train, y_test)


def load_Satimage_dataset(path='./dataset/Satimage/'):
    Satimage_train = pd.read_csv(path+'sat.trn.txt', header=None, delim_whitespace=True)
    input = shuffle(Satimage_train)
    x_train = input[np.arange(input.shape[1] - 1)]
    y_train = input[input.shape[1] - 1]

    Satimage_test = pd.read_csv(path + 'sat.tst.txt', header=None, delim_whitespace=True)
    input = shuffle(Satimage_test)
    x_test = input[np.arange(input.shape[1] - 1)]
    y_test = input[input.shape[1] - 1]
    return DataSet(x_train, x_test, y_train, y_test)


def load_Vowel_dataset(path='./dataset/Vowel/'):
    Vowel = pd.read_csv(path+'vowel-context.data.txt', header=None, delim_whitespace=True)

    # train_set selection, and remove non-features
    temp = Vowel[Vowel[0] == 0]
    x_train = temp[np.arange(3, temp.shape[1]-1)]
    y_train = temp[temp.shape[1]-1]

    # test_set selection, and remove non-features
    temp = Vowel[Vowel[0] == 1]
    x_test = temp[np.arange(3, temp.shape[1]-1)]
    y_test = temp[temp.shape[1]-1]
    return DataSet(x_train, x_test, y_train, y_test)st = pd.read_csv(path + 'optdigits.tes.txt', header=None)
    input = shuffle(optdigits_test)
    x_test = input[np.arange(input.shape[1] - 1)]
    y_test = input[input.shape[1] - 1]
    return DataSet(x_train, x_test, y_train, y_test)
...
def load_Vowel_dataset(path='./dataset/Vowel/'):
    ...
    x_test = temp[np.arange(3, temp.shape[1]-1)]
    y_test = temp[temp.shape[1]-1]
    return DataSet(x_train, x_test, y_train, y_test)ls_rest_part = self._labels[start:self._num_examples]
            # Shuffle the data
            if shuffle:
